...
// 配置静态文件路由
app.use(Middles.static(['/*/build/**/*', '/*/static/**/*'], {
...
let vhosts = Object.keys(config.vhost).map((item) => {
  ...
  // 在开发环境才使用mock数据功能
  config.mock && vapp.use(Middles.mock(vapp, {
    root: appPath + '/mock/',
  ...
  if (config.assets && config.assets[appName]
      && (config.assets[appName].resource || config.assets[appName].manifest)) {
    locals.reactCss = assets.reactCss(appName);
    locals.reactJs = assets.reactJs(appName);
  }
